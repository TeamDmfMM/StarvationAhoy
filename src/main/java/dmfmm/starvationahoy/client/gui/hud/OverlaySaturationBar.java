package dmfmm.starvationahoy.client.gui.hud;
...
public class OverlaySaturationBar extends Gui {
	...
	private static final int SATURATION_BAR_X = ConfigHandler.getOSX();
	private static final int SATURATION_BAR_Y = ConfigHandler.getOSY();
	private static final String SATURATION_TEXT_UNLOCALIZED = "hud.statarmor.saturation";
	
	private static final int EXHAUSTION_BAR_X = ConfigHandler.getOSX();
	private static final int EXHAUSTION_BAR_Y = ConfigHandler.getOSY() + 7;
	...
	@SubscribeEvent(priority = EventPriority.NORMAL)
	  public void onRenderExperienceBar(RenderGameOverlayEvent event)
	  {
		...
		for (int i : new int[] {3,2}){
			...
			if (i == 3 && this.mc.player.inventory.armorInventory.get(i).getItem() == CoreItemRegistry.STAT_HELM){
				count += 1;
				continue;
			}
			if (i == 2 && this.mc.player.inventory.armorInventory.get(i).getItem() == CoreItemRegistry.STAT_CHEST){
				count += 1;
				continue;
			}
		}
